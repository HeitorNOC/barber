...
const formSchema = z.object({
    name: z.string().min(2, { message: 'O campo deve conter no mínimo 2 caracteres.' }).max(50, { message: 'O campo deve conter no máximo 50 caracteres.' }),
    email: z.string().email({ message: 'Email inválido' }),
    senha: z.string().min(4, { message: 'O campo deve conter no mínimo 4 caracteres.' }).max(20, { message: 'O campo deve conter no máximo 20 caracteres.' }),
    celular: z.string().refine(celular => /^\(\d{2}\)\d\d{4}-\d{4}$/.test(celular), {
    ...
    endereço: z.object({
        ...
        rua: z.string().min(3, { message: 'O campo deve conter no mínimo 3 caracteres.' }).max(30, { message: 'O campo deve conter no máximo 30 caracteres.' }),
        numero: z.number(),
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 w-[500px]">
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormControl>
                                                    <Input placeholder="celular" {...field} isMask mask="(99)99999-9999" />
                                                </FormControl>
                                                <FormDescription>
                                                    This is your public display name.
                                                </FormDescription>
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormControl>
                                                    <Input placeholder="CEP" {...field} isMask mask="99999-999" onBlur={handleCheckCep} />
                                                </FormControl>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                                            ...
                                            <FormItem>
                                                ...
                                                >
                                                    ...
                                                    <SelectContent >
                                                        {
                                                            states.map((state) => (
                                                                <SelectItem {...field} key={state.estado} value={state.estado} >{state.estado}</SelectItem>
                                                            ))
                                                        }
                                                    </SelectContent>
                                                </Select>
                                                    ...
                                                    <SelectContent >
                                                        {
                                                            cidades ? (
                                                                cidades.map((cidadeAtual) => (
                                                                    <SelectItem {...field} key={String(cidadeAtual)} value={String(cidadeAtual)} >{cidadeAtual}</SelectItem>
                                                                ))
                                                            ) : (
                                                                <>
                                                                    <p>Não foi possível encontrar as cidades do seu estado, digite-a por favor.</p>
                                                                    <Input disabled={!estado} placeholder="Digite sua cidade" {...field} onChange={handleSetCidade} />
                                                                </>
                                                            )
                                                        }
                                                    </SelectContent>
                                                    ...
                                                    <SelectContent>
                                                        {
                                                            bairros ? (
                                                                bairros.map((bairroAtual) => (
                                                                    <SelectItem {...field} key={String(bairroAtual)} value={String(bairroAtual)} >{bairroAtual}</SelectItem>
                                                                ))
                                                            ) : (
                                                                <>
...

                        {
                            progress == 50 ? (
                                ...
                                <>
                                    <Button onClick={handleBackStep} className="mr-4">Voltar</Button>
